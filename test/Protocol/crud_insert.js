...
describe('Protocol', function () {
    describe('crudInsert', function () {
        ...
        it('should send one document in one message', function () {
            ...
            JSON.parse(DataType.decodeAny(data.decoded.row[0].field[0])).should.deep.equal({ _id: 123 });
        });
        it('should send multiple documents in one message', function () {
            ...
            JSON.parse(DataType.decodeAny(data.decoded.row[0].field[0])).should.deep.equal({ _id: 123 });
            JSON.parse(DataType.decodeAny(data.decoded.row[1].field[0])).should.deep.equal({ _id: 456 });
        });
