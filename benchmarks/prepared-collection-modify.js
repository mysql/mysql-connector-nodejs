...
const label = `prepared-collection-modify benchmark (${times} times)`;
...
const benchmark = async () => {
    ...
    try {
        ...
        const collection = await schema.createCollection('benchmark_prepared_collection_modify', { reuseExisting: true });
        ...
        console.time(label);
        const statement = collection.modify('firstName = :name')
            .arrayAppend('updatedAt', mysqlx.expr('current_timestamp()'))
            .set('name', mysqlx.expr("concat(firstName, ' ', lastName)"));
