...
const label = `replace-one benchmark (${times} times)`;
...
const benchmark = async () => {
    ...
    try {
        ...
        const collection = await schema.createCollection('benchmark_replace_one', { reuseExisting: true });
        const res = await collection.add(doc)
            .execute();

        console.time(label);
        await Promise.allSettled([...Array(times)].map((_, i) => collection.replaceOne(res.getGeneratedIds()[0], { times: i + 1 })));
