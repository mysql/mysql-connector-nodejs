...
const label = `get-one benchmark (${times} times)`;
...
const benchmark = async () => {
    ...
    try {
        ...
        const collection = await schema.createCollection('benchmark_get_one', { reuseExisting: true });
        const res = await collection.add(doc)
            .execute();

        console.time(label);
        await Promise.allSettled([...Array(times)].map(() => collection.getOne(res.getGeneratedIds()[0])));
