...
const label = `prepared-collection-find benchmark (${times} times)`;
...
const benchmark = async () => {
    ...
    try {
        ...
        const collection = await schema.createCollection('benchmark_prepared_collection_find', { reuseExisting: true });

        const tests = [...Array(times)].map((_, i) => docs[i % docs.length].firstName);

        console.time(label);
        const statement = collection.find('firstName = :name')
