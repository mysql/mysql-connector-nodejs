...
const tableName = 'benchmark_table_select';
const times = parseInt(process.argv[2]) || 10000;
const label = `table-select benchmark (${times} times)`;
...
const benchmark = async () => {
    ...
    try {
        ...
        const tests = [...Array(times)].map(() => {
            return table.select(mysqlx.expr('{ "name": concat(firstName, lastName), "year": year(curdate()) - age }'))
