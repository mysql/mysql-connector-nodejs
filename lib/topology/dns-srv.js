...
exports.lookup = function (serviceDefinition) {
    return dns.resolveSrv(serviceDefinition)
        ...
        .catch(err => {
            ...
            err.message = util.format(errors.MESSAGES.ERR_SRV_RECORDS_NOT_AVAILABLE, serviceDefinition);
            throw err;
        });
};
...
exports.validate = function ({ endpoints, host, port, resolveSrv, socket }) {
    ...
    if (!isValidBoolean({ value: resolveSrv })) {
        throw new Error(errors.MESSAGES.ERR_SRV_LOOKUP_INVALID_OPTION);
    }
    ...
    if (resolveSrv && endpoints && endpoints.length > 1) {
        throw new Error(errors.MESSAGES.ERR_SRV_LOOKUP_WITH_MULTIPLE_ENDPOINTS);
    }
    ...
    if (resolveSrv && endpoint.port) {
        throw new Error(errors.MESSAGES.ERR_SRV_LOOKUP_WITH_PORT);
    }
    ...
    if (resolveSrv && endpoint.socket) {
        throw new Error(errors.MESSAGES.ERR_SRV_LOOKUP_WITH_LOCAL_SOCKET);
    }
