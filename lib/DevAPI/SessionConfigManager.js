...
function SessionConfigManager () {
    ...
    return {
         ...
         */
        save (name, input, appdata) {
            if (!state.persistenceHandler) {
                return Promise.reject(new Error('No IPersistenceHandler implementation available'));
            }

            if (isSessionConfig(session)) {
                return saveConfig(session);
            }
            ...
            if (typeof input === 'string') {
                // input is a URI string
                return saveConfig(sessionConfig(session, input, appdata));
            }

            return saveConfig(sessionConfig(session, stringifyUri(input), mergeAppData(input)));
        },
