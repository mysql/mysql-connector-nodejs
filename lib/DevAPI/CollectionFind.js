...
function CollectionFind (session, schemaName, tableName, criteria) {
    ...
    return Object.assign({}, binding({ criteria }), collectionOrdering(), grouping(), limiting(), locking(), projecting(), query({ schemaName, session, tableName }), {
        /**
         ...
         * @param {module:CollectionFind~documentCursor} [rowcb] - callback function to handle results
         * @return {Promise.<module:Result>}
         */
        execute (rowcb) {
            const cursor = rowcb ? row => rowcb(row[0]) : undefined;

            return this.getSession()._client.crudFind(this, cursor).then(state => result(state));
        },
