...
const tlsVersions = require('../tls/versions');
 ...
 * @prop {string[]} [versions=TLSv1, TLSv1.1, TLSv1.2, TLSv1.3] - restrict the list of allowed TLS versions (TLSv1, TLSv1.1, TLSv1.2, TLSv1.3)
...
function deprecate ({ dbPassword, dbUser, ssl, sslOptions }) {
    ...
    if (typeof dbUser !== 'undefined') {
        deprecated(warnings.MESSAGES.WARN_DEPRECATED_DB_USER);
    }

    // The "dbPassword" property is deprecated.
    if (typeof dbPassword !== 'undefined') {
        deprecated(warnings.MESSAGES.WARN_DEPRECATED_DB_PASSWORD);
    }

    // The "ssl" and "sslOptions" properties are deprecated.
    if (typeof ssl !== 'undefined') {
        deprecated(warnings.MESSAGES.WARN_DEPRECATED_SSL_OPTION);
    }

    if (typeof sslOptions !== 'undefined') {
        deprecated(warnings.MESSAGES.WARN_DEPRECATED_SSL_ADDITIONAL_OPTIONS);
    }
    ...
    return {
        ...
        authenticate () {
            ...
            if (!this.allowsAuthenticationWith(mechanism)) {
                const message = util.format(errors.MESSAGES.ERR_AUTH_UNSUPPORTED_SERVER, mechanism);
                const error = new Error(message);
                error.info = { code: errors.ERR_AUTHENTICATION_FAILED, msg: message };
            ...
            return this.authenticateWith(mechanism)
                .catch(err => {
                    ...
                    // should report it.
                    if (!this.allowsAuthenticationRetry() || (err.info && err.info.code !== errors.ERR_AUTHENTICATION_FAILED)) {
                        throw err;
                    }
                    ...
                    return this.authenticateWith('SHA256_MEMORY')
                        .catch(err => {
                            ...
                            // error instead.
                            if (err.info && err.info.code !== errors.ERR_AUTHENTICATION_FAILED) {
                                throw err;
                            }

                            // Otherwise, we improve the existing error message.
                            err.message = err.info.msg = errors.MESSAGES.ERR_AUTH_MORE_INFO;
        ...
        capabilitiesSet () {
            ...
            return state.client.capabilitiesSet(capabilities)
                ...
                .catch(err => {
                    ...
                    if (err.info && err.info.code === errors.ERR_CAPABILITY_PREPARE_FAILED) {
                        ...
                        const message = errors.MESSAGES.ERR_TLS_DISABLED_IN_SERVER;
                        err.message = message;
                    ...
                    if (!err.info || err.info.code !== errors.ERR_UNKNOWN_CAPABILITY) {
                        throw err;
                    }

                    // By this point, the server has sent back an
                    // ERR_UNKNOWN_CAPABILITY error and has closed the
                    // connection.
            ...
            return new Promise((resolve, reject) => {
                ...
                nodeSocket.once('timeout', () => {
                    ...
                    if (!this.hasMultipleEndpoints()) {
                        error.message = util.format(errors.MESSAGES.ERR_SINGLE_HOST_CONNECTION_TIMEOUT, connectTimeout);
                    } else {
                        error.message = util.format(errors.MESSAGES.ERR_MULTI_HOST_CONNECTION_TIMEOUT, connectTimeout);
                    }
                ...
                nodeSocket.once('close', hasError => {
                    ...
                    if (!this.hasMoreEndpointsAvailable() && this.hasMultipleEndpoints()) {
                        ...
                        error.errno = errors.ERR_MULTI_HOST_CONNECTION_FAILURE;
                        error.message = errors.MESSAGES.ERR_MULTI_HOST_CONNECTION_FAILURE;
        ...
        enableTLS () {
            return new Promise(resolve => {
                ...
                nodeSocket.once('secureConnect', () => {
                    const tlsVersion = nodeSocket.getProtocol();
                    // Check if the connection is using a deprecated TLS version.
                    if (tlsVersions.deprecated().indexOf(tlsVersion) > -1) {
                        // If it is, a warning should be generated and logged
                        // or printed to stdout.
                        log('connection:tls').warning('version', util.format(warnings.MESSAGES.WARN_DEPRECATED_TLS_VERSION, tlsVersion), { type: 'DeprecationWarning' });
                    }
        ...
        getError () {
            ...
            return state.error || new Error(errors.MESSAGES.ERR_CONNECTION_CLOSED);
        },
...
Connection.validate = function ({ connectionAttributes, connectTimeout, dbUser, dbPassword, endpoints, host, port, resolveSrv, socket, ssl, sslOptions, tls }) {
    ...
    if (ports.some(p => !isValidInteger({ value: p, min: 0, max: 65536 }))) {
        throw new Error(errors.MESSAGES.ERR_INVALID_PORT_RANGE);
    }
    ...
    if (!isValidInteger({ value: connectTimeout, min: 0 })) {
        throw new Error(errors.MESSAGES.ERR_INVALID_CONNECTION_TIMEOUT_VALUE);
    }
    ...
    if (!isValidBoolean({ value: connectionAttributes }) && !isValidPlainObject({ value: connectionAttributes })) {
        throw new Error(errors.MESSAGES.ERR_INVALID_CONNECTION_ATTRIBUTES_DEFINITION);
    }
    ...
    if (Object.keys(Object.assign({}, connectionAttributes)).some(key => key.startsWith('_'))) {
        throw new Error(errors.MESSAGES.ERR_INVALID_CONNECTION_ATTRIBUTES_CONVENTION);
    }
