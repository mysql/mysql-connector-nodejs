...
function Schema (session, name) {
    ...
    return Object.assign({}, databaseObject(session), {
        ...
        getCollections () {
            ...
            const callback = row => {
                // Trimming the content avoids issues when the content is right padded
                // (see lib/Protocol/Protobuf/Adapters.js#decodeOpaqueByteString())
                if (row[1] && row[1].trim() !== 'COLLECTION') {
                    return;
                }

                collections.push(this.getCollection(row[0]));
            };
        ...
        getTables () {
            ...
            const callback = row => {
                // Trimming the content avoids issues when the content is right padded
                // (see lib/Protocol/Protobuf/Adapters.js#decodeOpaqueByteString())
                if (row[1] && row[1].trim() !== 'TABLE' && row[1].trim() !== 'VIEW') {
                    return;
                }

                tables.push(this.getTable(row[0]));
            };
