...
Schema.prototype.getCollections = function () {
    ...
    return this._session._client.sqlStmtExecute("list_objects", args, row => {
        collections[row[0]] = this.getCollection(row[0]);
...
Schema.prototype.getTables = function () {
    ...
    return this._session._client.sqlStmtExecute("SELECT TABLE_NAME FROM information_schema.TABLES WHERE TABLE_CATALOG = ? AND TABLE_SCHEMA = ?", args, row => {
        tables[row[0]] = this.getTable(row[0]);
    }).then(function () {
        return tables;
    });
};
