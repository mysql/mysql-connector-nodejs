 ...
 */
function CollectionAdd (session, schema, collection, documents) {
    let state = { session, schema, collection, documents: documents || [] };
        ...
        execute () {
            ...
            const documents = state.documents.map(doc => {
                if (typeof doc._id === 'undefined') {
                    doc._id = state.session.idGenerator();
                }
            ...
            return state
                .session
                ._client
                .crudInsert(state.schema.getName(), state.collection, Client.dataModel.DOCUMENT, documents)
                .then(state => {
