...
function ConnectionPool (state) {
    ...
    return {
        ...
        acquire () {
            ...
            return this.refresh()
                .then(() => {
                    ...
                    if (!session._isValid && !session._isReusable) {
        ...
        pick () {
            ...
            const size = state.active.push(session);
            ...
            if (!state.enabled) {
                ...
                session.done = Session.prototype.close;
                session.close = () => new Promise(resolve => {
        ...
        start (uri) {
            ...
            for (let i = 0; i < state.maxSize; ++i) {
                const session = new Session(Object.assign({}, uri, { pooling: true }));
                session.done = session.close;
