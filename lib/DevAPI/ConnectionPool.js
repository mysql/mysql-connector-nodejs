...
function ConnectionPool (state) {
    ...
    return {
         ...
         */
        refresh () {
            const sessionsWithoutConnection = state.active.filter(session => !session._isOpen);

            if (state.maxIdleTime === 0) {
                // even if we don't close idle sessions, we can still close active sessions without an open connection
                return Promise.all(sessionsWithoutConnection.map(session => session.close()));
            }

            // otherwise, we should also close the idle sessions
            const idleSessions = state.idle.filter(session => session.timestamp && (Date.now() - session.timestamp > state.maxIdleTime));

            return Promise.all(idleSessions.concat(sessionsWithoutConnection).map(session => session.close()));
        },
