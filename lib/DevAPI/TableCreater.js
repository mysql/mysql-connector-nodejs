...
TableCreater.prototype.addPrimaryKey = function (field) {
    this._primary = Array.from(arguments);
    return this;
};
...
TableCreater.prototype.addIndex = function (indexName, field) {
    this._indexes.push({ name: indexName, fields: Array.from(arguments) });
    return this;
};
...
TableCreater.prototype.addUniqueIndex = function (indexName, field) {
    this._uniques.push({ name: indexName, fields: Array.from(arguments) });
    return this;
};
        ...
        (this._as ?
            ...
            (
                "(\n" +this._fields.map(f => f.get()).join(",\n") +
                   (this._foreignKey.length ? ", " + this._foreignKey.map(fk => fk.get()).join("\n") : "") +
                ")" +
                options.join(", ")
            )
        );

    return this._schema._session._client.sqlStmtExecute(sql).then(() => true);
};
