...
function _buildSchema () {
    ...
    // TODO(Rui): this will change when this feature gets a proper spec or gets dropped.
    return this.getSchema(this._properties.schema).existsInDatabase().then((exists) => {
        ...
        return this.createSchema(this._properties.schema)
            .catch(err => {
                // This is a hack to avoid errors in a pooling scenario.
                if (!err.info || err.info.code !== 1007) {
                    throw err;
                }
...
Session.prototype.inspect = function (depth) {
    const available = ['auth', 'dbUser', 'host', 'pooling', 'port', 'socket', 'ssl', 'user'];
