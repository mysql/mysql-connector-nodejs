...
function Session (properties) {
    ...
    // TODO(Rui): remove "dbPassword" after deprecation period
    if (properties.dbPassword) {
        deprecated('The "dbPassword" property is deprecated since 8.0.22 and will be removed in future versions. Use "password" instead.');
    }

    // TODO(Rui): remove "dbUser" after deprecation period
    if (properties.dbUser) {
        deprecated('The "dbUser" property is deprecated since 8.0.22 and will be removed in future versions. Use "user" instead.');
    }
    ...
    // connectionAttributes falsy values can have special meaning
    this._properties = Object.assign({}, { connectionAttributes: {} }, properties, {
        auth: properties.auth || ((properties.tls.enabled || properties.socket) && 'PLAIN'),
        connectTimeout: properties.connectTimeout || 10000,
        // remove after deprecation period
        dbUser: properties.user || properties.dbUser || '',
        host: properties.host || 'localhost',
        endpoints: properties.endpoints || [{
            host: properties.host ? properties.host : properties.socket ? undefined : 'localhost',
            port: properties.port ? properties.port : properties.socket ? undefined : 33060,
        ...
        socket: properties.socket || undefined,
        ssl: properties.tls.enabled,
        user: properties.user || properties.dbUser || ''
    });

    // Obfuscate password in dumps. Revisit this later if needed.
    Object.defineProperty(this._properties, 'password', {
        get: function () {
...
function _setConnectionDetails (endpoint) {
...
function _sendConnectionAttributes () {
    const attribs = this._properties.connectionAttributes;
 ...
 * @private
 */
function _connectionFailureHandler (err) {
    ...
    return createSocket.call(this, this._available[0]);
}
    ...
    return _createConnection.call(this, socket)
        .then(() => {
            return Promise.all([
                ...
                _authenticate.call(this).then(connection => {
        ...
        .catch(err => {
            return _connectionFailureHandler.call(this, err);
        });
}

function createSocket (connection) {
    return new Promise((resolve, reject) => {
        // if there is no endpoint available, there is nothing left to do
        if (!connection) {
            const error = new Error('Unable to connect to any of the target hosts.');
            error.errno = 4001;
        ...
        const options = !connection.socket ? connection : { path: connection.socket };
        const timeout = parseFloat(this._properties.connectTimeout);

        _setConnectionDetails.call(this, connection);
        ...
        socket.on('timeout', () => {
            ...
            _connectionFailureHandler.call(this, err).then(resolve).catch(reject);
        });
        ...
        socket.once('close', hasError => {
            ...
            if ((!hasError && !networkError) || this._isOpen || this._isValid) {
                return;
            }

            networkError = networkError || new Error('The server has gone away');

            _connectionFailureHandler.call(this, networkError).then(resolve).catch(reject);
        });
    });
}
...
Session.prototype.connect = function () {
    // in single-host setting, all we can do is try to create the socket for the given endpoint
    if (!this._properties.resolveSrv && this._properties.endpoints.length === 1) {
        // we need to make sure the single host is always retried
        return createSocket.call(this, this._properties.endpoints[0]);
    }

    // if it's neither a single-host setting or a DNS SRV setting, it's a multi-host setting
    // and, in that case, we need to sort the list of endpoints according to the format
    if (!this._properties.resolveSrv) {
        const now = Date.now();

        // in the case of a connection pool, the list of available endpoints should be up-to-date
        this._available = multihost.getSortedAddressList(this._available.concat(this._unavailable.filter(e => now - e.lastCheck >= MULTIHOST_RETRY)));
        this._unavailable = this._unavailable.filter(e => now - e.lastCheck < MULTIHOST_RETRY);

        // in the end, we need to try to connect to the first available endpoint
        return createSocket.call(this, this._available[0]);
    }

    // if it's a DNS SRV setting, we need to obtain the list of endpoints from the DNS
    // server and use them in a similar way to the multi-host setting
    return srv.getSortedAddressList(this._requestedHost)
        .then(endpoints => {
            this._available = endpoints.map(address => ({ host: address.name, port: address.port }));
