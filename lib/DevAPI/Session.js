...
Session.prototype.connect = function () {
    ...
    return srv.getSortedAddressList(this._requestedHost)
        .then(endpoints => {
            const availableHosts = endpoints.map(address => ({ host: address.name, port: address.port }));

            this._properties.endpoints = availableHosts;
            this._properties.host = availableHosts[0].host;
            this._properties.port = availableHosts[0].port;
