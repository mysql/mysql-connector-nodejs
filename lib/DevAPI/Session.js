...
function Session (connection) {
    ...
    return {
        ...
        dropSchema (name) {
            ...
            return this.sql(`DROP DATABASE ${Table.escapeIdentifier(name)}`)
                ...
                .catch(err => {
                    ...
                    if (!err.info || err.info.code !== errors.ERR_DATABASE_DOES_NOT_EXIST) {
                        throw err;
                    }
         ...
         */
        executeSql: util.deprecate(function (sql) {
            return this.sql(sql);
        }, warnings.MESSAGES.WARN_DEPRECATED_EXECUTE_SQL),
        ...
        releaseSavepoint (name) {
            if (typeof name !== 'string' || !name.trim().length) {
                return Promise.reject(new Error(errors.MESSAGES.ERR_INVALID_SAVEPOINT_NAME));
            }
