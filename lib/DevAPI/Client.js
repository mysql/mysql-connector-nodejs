...
function Client (options = {}) {
    ...
    return {
        ...
        close () {
            if (state.pool === null) {
                return Promise.reject(new Error(errors.MESSAGES.ERR_POOL_CLOSED));
            }
        ...
        getSession () {
            if (!options.pooling.enabled) {
                return connection(options).open()
                    .then(con => {
                        return session(con);
                    });
            }
...
Client.validate = function (options) {
    ...
    if (invalidKeys.length) {
        throw new Error(util.format(errors.MESSAGES.ERR_CLIENT_INVALID_OPTION, invalidKeys[0]));
    }

    if (!isValidPlainObject({ value: options.pooling })) {
        throw new Error(util.format(errors.MESSAGES.ERR_CLIENT_INVALID_OPTION_VALUE, 'pooling', options.pooling));
    }
