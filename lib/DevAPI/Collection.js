    ...
    const collection = Object.assign({}, databaseObject(connection), {
        ...
        addOrReplaceOne (id, data = {}) {
            ...
            if (typeof data._id !== 'undefined' && data._id !== id) {
                return Promise.reject(new Error(errors.MESSAGES.ERR_NON_MATCHING_ID_IN_REPLACEMENT_DOCUMENT));
            }
        ...
        dropIndex (name) {
            if (typeof name !== 'string' || !name.trim().length) {
                return Promise.reject(new Error(errors.MESSAGES.ERR_INVALID_INDEX_NAME));
            }
        ...
        createIndex (name, constraint) {
            ...
            if (!isValidDefinition) {
                return Promise.reject(new Error(errors.MESSAGES.ERR_INVALID_INDEX_DEFINITION));
            }

            if (constraint.unique === true) {
                return Promise.reject(new Error(errors.MESSAGES.ERR_UNIQUE_INDEX_NOT_SUPPORTED));
            }
