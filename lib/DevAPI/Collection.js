 ...
 * @returns {Collection}
 */
function Collection (session, schema, name, alias) {
    const state = Object.assign({}, { alias, name, schema });
    ...
    return Object.assign({}, databaseObject(session), {
        ...
        count () {
            const schema = table.escapeIdentifier(this.getSchema());
            const collection = table.escapeIdentifier(this.getName());

            let count = 0;
            let callback = row => { count = row[0]; };

            return stmtExecute(this.getSession(), `SELECT COUNT(*) FROM ${schema}.${collection}`)
                .execute(callback)
                .then(() => count);
        },
        ...
        existsInDatabase () {
            const args = [this.getSchema(), this.getName()];

            let status = false;
            let callback = found => { status = !!found.length; };
         ...
         */
        inspect () {
            return { schema: this.getSchema(), collection: this.getName() };
        },
        ...
        dropIndex (name) {
            ...
            // FIXME(Amitabh): this should use stmtExecute.Namespace.X_PLUGIN which in turn should specify the 'mysqlx' namespace (watch out for breaking changes on other APIs using this command, such as createCollection)
            return stmtExecute(this.getSession(), 'drop_collection_index', args, 'mysqlx')
                .execute()
                .then(() => true);
        },
        ...
        createIndex (name, constraint) {
            ...
            const args = [{
                ...
                constraint: constraint.fields.map(item => {
                    ...
                    let data = Object.assign({ required: false, member: item.field }, item);
                    delete data.field;
            ...
            // FIXME(Amitabh): this should use stmtExecute.Namespace.X_PLUGIN which in turn should specify the 'mysqlx' namespace (watch out for breaking changes on other APIs using this command, such as createCollection)
            return stmtExecute(this.getSession(), 'create_collection_index', args, 'mysqlx')
                .execute()
                .then(() => true);
        }
    });
