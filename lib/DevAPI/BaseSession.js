...
function _createConnection (connection) {
    ...
    // FIXME(rui.quelhas): this causes issues when enabling SSL, bug MYP-264.
    // Question: Do we really care about capabilities or can we simply be optimistic
    // in regards to auth an SSL mind: If we're pessimistic we'd actually have to
    // check capabilities, again, after enabling SSL as things change
    // (i.e. PLAIN auth becomes available)
    //
    ...
    return this._client.enableSSL(Object.assign({}, this._properties.sslOptions, { isServer: false }));
};
