...
/**
 * Sub-parser that matches a column identifier in table mode.
...
const parser = _ => r => Pa
    .seq(
        ...
        Pa.alt(Pa.string('->>'), Pa.string('->'))
            .then(Pa.string('$').then(r.documentPath).wrap(Pa.string("'"), Pa.string("'")))
            .atMost(1)
            .map(([documentPath = []]) => documentPath)
    )
    .map(([identifier, documentPath]) => ({ type: PARSER_OPTIONS.ID, value: { documentPath, ...identifier } }));
