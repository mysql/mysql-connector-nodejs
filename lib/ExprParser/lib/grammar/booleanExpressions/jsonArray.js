...
/**
 * Sub-parser that matches a JSON array.
 * jsonArray ::= '[' ( expr ( ',' expr )* )* ']'
 * @private
 * @param {Object} [_] - Optional object containing parser options.
 * @returns The generated grammar for the parser.
 * @see https://dev.mysql.com/doc/x-devapi-userguide/en/mysql-x-expressions-ebnf-definitions.html#expression-ebnf-jsonarray
 * @example
 * ["foo", 1, true, { "baz": 1.234 }, ["qux"]]
 */
const parser = _ => r => r.expr
    ...
    // r.expr returns a list of objects with a "value" key that contains the
    // corresponding list item value definition
    .map(items => items.map(({ value }) => value))
    .map(items => ({ type: PARSER_OPTIONS.ID, value: items }));
