...
const comparisonFunction = Pa.alt(
    Pa.string('<>').result('!='),
    Pa.string('>='),
    Pa.string('<='),
    Pa.string('!='),
...
/**
 * Concrete sub-parser that matches a comparison expression.
 ...
 */
const parser = _ => r => Pa
    .seq(r.bitExpr, Pa.seq(comparisonFunction.trim(Pa.optWhitespace), r.bitExpr).map(([name, param]) => ({ name, params: [param] })).many())
