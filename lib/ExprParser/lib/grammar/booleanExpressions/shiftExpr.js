...
const bitShiftingFunctions = Pa.alt(
    Pa.string('>>'),
    Pa.string('<<')
);

/**
 * Concrete sub-parser that matches a bit-shifting operator expression.
 ...
 * @see https://dev.mysql.com/doc/x-devapi-userguide/en/mysql-x-expressions-ebnf-definitions.html#expression-ebnf-shiftexpr
 * @example
 * 1 >> 2
 * 2 << 1
 */
const parser = _ => r => Pa
    .seq(r.addSubExpr, Pa.seq(bitShiftingFunctions.trim(Pa.optWhitespace), r.addSubExpr).map(([name, param]) => ({ name, params: [param] })).many())
