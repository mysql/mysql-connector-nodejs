...
const PARSER_OPTIONS = {
    ID: 'projectedSearchExpr',
    NAME: 'PROJECTED_SEARCH_EXPR'
};

/**
 * Sub-parser that matches a projected search expression.
 ...
 * count(foo) as count
 */
const parser = ({ input }) => r => Pa
    .seq(r.expr.map(e => e.value), r.AS.trim(Pa.whitespace).then(r.ident).atMost(1).map(([alias]) => alias))
    .map(([source, alias = input]) => ({ type: PARSER_OPTIONS.ID, value: { source, alias } }));
