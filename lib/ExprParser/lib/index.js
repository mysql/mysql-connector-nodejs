...
function parser (options) {
    ...
    const parsers = Object.assign({}, keywords(options), partials(options), booleanExpressions(options), collectionOrTableExpressions(options), {
        ...
        QUOTED_ID (_) {
            return Pa.alt(Pa.string('``').result('`'), Pa.noneOf('`'))
                .many()
                .tie()
        ...
        STRING_DQ (r) {
            return Pa.alt(r.SCHAR, Pa.string("'"), r.ESCAPED_DQ, r.ESCAPED_ESCAPE_CHAR)
                .many()
                .wrap(Pa.string('"'), Pa.string('"'))
                .tie();
        },
        ...
        ESCAPED_DQ () {
            return Pa.alt(Pa.string('""'), Pa.string('\u005C"')).result('"');
        },

        ESCAPED_ESCAPE_CHAR (r) {
            return Pa.string('\u005C\u005C').result('\u005C');
        },

        STRING_SQ (r) {
            return Pa.alt(r.SCHAR, Pa.string('"'), r.ESCAPED_SQ, r.ESCAPED_ESCAPE_CHAR)
                .many()
                .wrap(Pa.string("'"), Pa.string("'"))
                .tie();
        },
