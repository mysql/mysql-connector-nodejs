...
const NoticeScope = require('./Stubs/mysqlx_notice_pb').Frame.Scope;
...
Client.prototype.handleServerMessage = function (message) {
    ...
    if (decoded.id === notice.MESSAGE_ID) {
        ...
        if (frame.toObject().scope === NoticeScope.GLOBAL) {
            return log('protocol:inbound:Mysqlx.Notice').info('Frame', frame);
        }
    }
