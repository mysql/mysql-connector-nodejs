...
Client.prototype.enableSSL = function (options) {
    ...
    return this
        ...
        .then(security => new Promise((resolve, reject) => {
            ...
            // TODO(Rui): This does not belong here. Should be extracted to a proper connection context.
            this._stream = tls.connect(config, () => resolve(this._stream));
            this._stream.on('error', () => { this._session._isValid = false; this._session._isOpen = false; });
            this._stream.on('end', () => { this._session._isValid = false; this._session._isOpen = false; });
        }))
        .then(stream => {
...
Client.prototype.sessionReset = function () {
    ...
    try {
        protobuf = this.encodeMessage(ClientMessages.Type.SESS_RESET, Session.encodeReset());
    } catch (err) {
        return Promise.reject(err);
    }
    ...
    return handler.sendMessage(this._workQueue, this._stream, protobuf)
        .then(() => this.authenticate(this.authenticator));
};
