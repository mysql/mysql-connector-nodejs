...
function Client (stream, session) {
    ...
    this._connectionError = errors.MESSAGES.ERR_CONNECTION_CLOSED;
    ...
    return this.capabilitiesSet({ tls: true })
        ...
        .catch(err => {
            if (err.info && err.info.code === 5001) {
                err.message = errors.MESSAGES[errors[err.info.code]];
            }
...
Client.prototype.decodeMessage = function (data) {
    ...
    if (isWireProtocol) {
        throw new Error(errors.MESSAGES.ERR_WIRE_PROTOCOL_HEADER);
    }

    const header = this.decodeMessageHeader(data);

    if (data.length < header.packetLength) {
        throw new Error(errors.MESSAGES.ERR_INCOMPLETE_PROTOCOL_MESSAGE);
    }
...
Client.prototype.decodeMessageHeader = function (data) {
    if (data.length < 4 /* header size */ + 1 /* type flag size */) {
        throw new Error(errors.MESSAGES.ERR_UNKNOWN_PROTOCOL_HEADER);
    }
        ...
        if (frame.getScopeId() === NoticeStub.Frame.Scope.GLOBAL) {
            ...
            if (frame.getTypeId() === NoticeStub.Frame.Type.WARNING) {
                ...
                if (payload.getLevelId() === NoticeStub.Warning.Level.ERROR) {
                    this._connectionError = errors.MESSAGES[errors[payload.getCode()]];
                }
            }
...
Client.prototype.sendConnectionAttributes = function (userAttributes) {
    ...
    const clientAttributes = tools.getSystemAttributes();
...
Client.prototype.crudInsert = function (query) {
    ...
    if (!query.getSession()._isValid) {
        return Promise.reject(new Error(this._connectionError));
    }
