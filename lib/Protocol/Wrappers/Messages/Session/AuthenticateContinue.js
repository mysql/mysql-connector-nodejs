...
AuthenticateContinue.create = function (token) {
    ...
    proto.setAuthData(bytes.deserialize(token).valueOf());
...
AuthenticateContinue.deserialize = function (buffer) {
    return AuthenticateContinue(AuthenticateContinueStub.deserializeBinary(bytes.deserialize(buffer).valueOf()));
};
