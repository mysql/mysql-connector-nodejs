...
pObject.create = function ({ fields, isLiteral, placeholders }) {
    ...
    proto.setFldList(Object.keys(fields).map(key => pObjectField.create({ isLiteral, key, placeholders, value: fields[key] }).valueOf()));
...
pObjectField.create = function ({ isLiteral, key, placeholders, value }) {
    ...
    proto.setKey(key);
    proto.setValue(Expr.create({ isLiteral, placeholders, value }).valueOf());
