 ...
 */
function decodeOpaqueByteString (reader, { isBinary = false, isJSON = false, padding = 0, length = 0 } = {}) {
 ...
 * @returns {number|string} A JavaScript number or string (to avoid loosing precision) containing the decimal value.
 */
function decodeSignedInt (reader) {
    const [signedIntString] = reader.readPackedSint64String();
    const signedInt = BigInt(signedIntString);

    if (signedInt >= Number.MIN_SAFE_INTEGER && signedInt <= Number.MAX_SAFE_INTEGER) {
        return Number(signedInt);
    }

    // TODO(Rui): we will eventually add support for BigInt as well.
 ...
 */
function decodeUnsignedInt (reader, { padding = 0 } = {}) {
    const [unsignedIntString] = reader.readPackedUint64String();
    const unsignedInt = BigInt(unsignedIntString);

    if (!padding && unsignedInt <= Number.MAX_SAFE_INTEGER && unsignedInt >= Number.MIN_SAFE_INTEGER) {
        return Number(unsignedInt);
    }
...
function decodeField (field, options) {
    ...
    if (type === ResultsetStub.ColumnMetaData.FieldType.FLOAT) {
        ...
        data = decodeSignedInt(reader);
    } else if (type === ResultsetStub.ColumnMetaData.FieldType.UINT) {
        data = decodeUnsignedInt(reader, { padding: column.isFlagged() && column.getLength() });
        ...
        data = decodeOpaqueByteString(reader, { isBinary: column.isBinary(), isJSON: column.isJSON(), length: field.length, padding: column.isFlagged() && column.getLength() });
