...
/**
 * Create a wrapper for a Mysqlx.Prepare.Prepare.OneOfMessage protobuf message
 * given a statement instance.
 * @param {Preparing} statement
 * @returns {module:adapters.Mysqlx.Prepare.Prepare.OneOfMessage}
 */
OneOfMessage.create = function (statement) {
    const proto = new OneOfMessageStub();

    const type = statement.getType();
    proto.setType(type);

    // We are in the stage where the statement is, in fact, prepared so
    // we need to ensure that the corresponding protobuf message does not
    // include any placeholder value.
    const options = { toPrepare: true };
