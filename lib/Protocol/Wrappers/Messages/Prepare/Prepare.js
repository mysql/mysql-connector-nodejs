...
/**
 * Create a wrapper for a Mysqlx.Prepare.Prepare protobuf message given a
 * statement instance.
 * @param {Preparing} statement
 * @returns {module:adapters.Mysqlx.Prepare.Prepare}
 */
Prepare.create = function (statement) {
    const proto = new PrepareStub();
    proto.setStmtId(statement.getStatementId());
    proto.setStmt(oneOfMessage.create(statement, { toPrepare: true }).valueOf());
