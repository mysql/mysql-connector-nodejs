...
const ServerMessages = require('../Stubs/mysqlx_pb').ServerMessages.Type;
...
exports.decodeOk = function (data) {
    const proto = Ok.deserializeBinary(tools.createTypedArrayFromBuffer(data));
...
exports.decodeError = function (data) {
    const proto = ServerError.deserializeBinary(tools.createTypedArrayFromBuffer(data));
...
/**
 * Extended enum object containting server message ids.
 * Adds an id used internally by the client to check if the server has closed the socket.
 */
exports.ServerMessages = Object.assign({}, ServerMessages, { SERVER_GONE: -1 });
