...
function createOctets (datatype) {
    ...
    bin.setValue(tools.createTypedArrayFromBuffer(datatype));
...
function createString (datatype) {
    ...
    // eslint-disable-next-line node/no-deprecated-api
    str.setValue(tools.createTypedArrayFromBuffer(new Buffer(datatype)));
...
exports.extractScalar = function (proto) {
    ...
    case Datatypes.Scalar.Type.V_STRING:
        ...
        return tools.createBufferFromTypedArray(proto.getVString().getValue()).toString();
    default:
        throw new Error('Invalid datatype for Mysqlx.Datatypes.Scalar.');
    }
};
