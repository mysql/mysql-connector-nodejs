...
function decodeField (field, options) {
    ...
    switch (options.column.getTypeId()) {
    case ColumnMetaDataStub.FieldType.FLOAT:
        return decodeFloat(reader, options);
    case ColumnMetaDataStub.FieldType.DOUBLE:
        return decodeDouble(reader, options);
    case ColumnMetaDataStub.FieldType.SINT:
        return decodeSignedInt(reader, options);
    case ColumnMetaDataStub.FieldType.UINT:
        return decodeUnsignedInt(reader, options);
    case ColumnMetaDataStub.FieldType.BIT:
        return decodeBitSequence(reader, options);
    case ColumnMetaDataStub.FieldType.BYTES:
    case ColumnMetaDataStub.FieldType.ENUM:
        return decodeOpaqueByteString(reader, Object.assign({ length: field.length }, options));
    case ColumnMetaDataStub.FieldType.TIME:
        return decodeTime(reader, options);
    case ColumnMetaDataStub.FieldType.DATETIME:
        return decodeDatetime(reader, options);
    case ColumnMetaDataStub.FieldType.DECIMAL:
        return decodeDecimal(reader, options);
    case ColumnMetaDataStub.FieldType.SET:
...
function decodeOpaqueByteString (reader, options) {
    ...
    if ((!options.column.getContentType() && options.column.getCharset() === 'binary') || options.column.getContentType() === ContentType.GEOMETRY) {
    ...
    if (options.column.getContentType() !== ContentType.JSON) {
        return data;
    }
...
exports.decodeRow = function (data, options) {
    ...
    return proto
        .getFieldList()
        .map((field, index) => decodeField(field, { column: options.metadata[index] }));
};
