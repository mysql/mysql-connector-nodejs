...
function decodeOpaqueByteString (reader, options) {
    ...
    options = Object.assign({ metadata: { contentType: ContentType.XML } }, options);

    if (!options.length) {
        return null;
    }

    if (options.metadata.contentType === ContentType.GEOMETRY) {
...
function decodeUnsignedInt (reader, options) {
    ...
    return leftPad(uint, options.metadata.length);
}
...
function formatDatetime (temporal, options) {
    const year = leftPad(temporal[0], 4);
    const month = leftPad(temporal[1], 2);
    const day = leftPad(temporal[2], 2);
...
function formatTime (temporal, options) {
    ...
    const hour = leftPad(temporal[1], 2);
    const minutes = leftPad(temporal[2], 2);
    const seconds = leftPad(temporal[3], 2);
    const useconds = leftPad(temporal[4], 6);
...
function leftPad (value, size) {
    if (Buffer.isBuffer(value)) {
        size = size && size > value.length ? size : value.length;
    ...
    const char = !isNaN(parseInt(value), 10) ? '0' : ' ';
