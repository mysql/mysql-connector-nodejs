...
function decodeDecimal (decoder, options) {
    const bytes = tools.createBufferFromTypedArray(decoder.readBytes(options.length));
...
function decodeOpaqueByteString (decoder, options) {
    ...
    if (options.column.isBinary()) {
        ...
        const data = tools.createBufferFromTypedArray(decoder.readBytes(options.length - 1));
...
function decodeSet (decoder, options) {
    ...
    const first = tools.createBufferFromTypedArray(decoder.readBytes(1));
    ...
    if (first.toString('hex') === '01') {
        try {
            return group.concat(tools.createBufferFromTypedArray(decoder.readBytes(1)).toString());
        } catch (err) {
...
function decodeSignedInt (decoder, options) {
    ...
    return formatSafeNumber(decoder.readZigzagVarint64String());
}
 ...
 * @private
 * @alias module:Row
 * @param {Mysqlx.Resultset.Row} proto - protobuf stub
 * @returns {module:Row}
 */
function Row (proto, options) {
    ...
    return {
         ...
         * @private
         * @function
         * @name module:Row#getColumnMetadata
         * @returns {Array<module:ColumnMetadata>} The array of column metadata type instances.
         */
        getColumnMetadata () {
         ...
         * @private
         * @function
         * @name module:Row#setColumnMetadata
         * @returns {module:Row} The Row instance.
         */
