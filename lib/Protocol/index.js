 ...
 * @constructor
 */
function Protocol(stream, globals) {
    this._stream = stream;
    ...
    this.serverMessages[Messages.ServerMessages.PARAMETER_CHANGED_NOTIFICATION] = "Mysqlx.ParameterChangedNotification";
    this.serverMessages[Messages.ServerMessages.CONN_CAPABILITIES] = "Mysqlx.Connection.Capabilities";
    this.serverMessages[Messages.ServerMessages.SESS_AUTHENTICATE_CONTINUE] = "Mysqlx.Session.AuthenticateContinueXXX";
    this.serverMessages[Messages.ServerMessages.SESS_AUTHENTICATE_OK] = "Mysqlx.Session.AuthenticateOk";
    this.serverMessages[Messages.ServerMessages.SQL_PREPARE_STMT_OK] = "Mysqlx.SQL_PREPARE_STMT_OKXXX";
    this.serverMessages[Messages.ServerMessages.SQL_PREPARED_STMT_EXECUTE_OK] = "Mysqlx.SQL_PREPARED_STMT_EXECUTE_OKXXX";
    ...
    this.serverMessages[Messages.ServerMessages.SQL_CURSOR_FETCH_DONE] = "Mysqlx.Sql.CursorFetchDone";
    this.serverMessages[Messages.ServerMessages.SQL_CURSOR_FETCH_SUSPENDED] = "Mysqlx.SQL_CURSOR_FETCH_SUSPENDEDXXX";
    this.serverMessages[Messages.ServerMessages.SQL_CURSORS_POLL] = "Mysqlx.SQL_CURSORS_POLLXXX";
    this.serverMessages[Messages.ServerMessages.SQL_CURSOR_CLOSE_OK] = "Mysqlx.SQL_CURSOR_CLOSE_OKXXX";
    this.serverMessages[Messages.ServerMessages.SQL_CURSOR_FETCH_DONE_MORE_RESULTSETS] = "Mysqlx.Sql.CursorFetchDoneMoreResultsets";
 ...
 * @param data
 */
Protocol.prototype.encodeMessage = function (messageId, data) {
    data = data || {};
    var buffer = protobuf.encode(this.clientMessages[messageId], data, 5, 5);
...
Protocol.prototype.handleServerMessage = function (message) {
    this._globals.getLogger().log('got %d bytes of data', message.length);
    var payloadLen = 0;
    for (var offset = 0; offset < message.length; offset += payloadLen) {
        // The length we report to the server does not include the length of the length field
        payloadLen = message.readUInt32LE(offset) + 4;
        var slice = message.slice(offset + 5, offset + payloadLen);

        var messageId = message[offset + 4];
        var messageName = this.serverMessages[messageId];
        var protoBuffer = slice;

        var decoded = protobuf.decode(messageName, protoBuffer);

        if (messageId == Messages.ServerMessages.NOTICE || messageId == Messages.ServerMessages.PARAMETER_CHANGED_NOTIFICATION) {
            console.log("Need to handle out-of-band message!");
            throw new Error("Need to handle out-of-band message!");
        }

        this._workQueue.process(messageId, decoded);
    }
};
...
Protocol.prototype.handshake = function (properties) {
...
Protocol.prototype.authenticateContinue = function (auth_data) {
    var data = {
        auth_data: auth_data
    };
    var buffer = this.encodeMessage(Messages.ClientMessages.SESS_AUTHENTICATE_CONTINUE, data);
    this.sendDirect(this._stream, buffer);
};
