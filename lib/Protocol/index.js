...
Protocol.prototype.enableSSL = function (options) {
    var self = this;
    options = options || {};
    options.isServer = false;

    return this.capabilitiesSet({tls: true}).then(function () {
        var tls = require('tls');
        self._stream = new tls.TLSSocket(self._stream, options);
        self._stream.on('data', function (data) { return self.handleServerMessage(data); });
        self._stream.on('close', function () { return self.handleServerClose(); });
        return true;
    });
};
...
var sessionStateParameters = {};
Object.getOwnPropertyNames(Messages.messages[noticeDecoders[3]].enums.Parameter).forEach(function (name) {
...
Protocol.prototype.capabilitiesSet = function (properties) {
    var data = {
        capabilities: {
            capabilities: []
        }
    ...
    var buffer = this.encodeMessage(Messages.ClientMessages.CON_CAPABILITIES_SET, data);

    var handler = new handlers.OkHandler(this);
    return handler.sendMessage(this._workQueue, this._stream, buffer);
};
...
Protocol.prototype.crudInsert = function (schema, collection, model, rows, projection) {
    ...
    rows.forEach(function (row) {
        var fields = row.map(function (field) {
            if (model === Protocol.dataModel.DOCUMENT) {
                field = JSON.stringify(field);
            }
            return {
                    type: Messages.messages['Mysqlx.Expr.Expr'].enums.Type.LITERAL,
                    literal: dt.encodeScalar(field)
            };
        });
...
Protocol.prototype.crudFind = function (session, schema, collection, model, projection, criteria, limit, rowcb, metacb) {
    var data = {
        collection: {
            schema: schema,
            name: collection
...
Protocol.prototype.crudModify = function (session, schema, collection, dataModel, criteria, operations) {
    var data = {
        collection: {
            schema: schema,
            name: collection
...
Protocol.prototype.crudRemove = function (session, schema, collection, model, criteria, limit, rowcb, metacb) {
    var data = {
        collection: {
            schema: schema,
            name: collection
