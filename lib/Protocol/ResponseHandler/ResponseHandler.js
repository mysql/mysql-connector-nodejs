...
ResponseHandler.prototype.sendMessage = function (queue, stream, buffer) {
    ...
    return new Promise(function (resolve, fail) {
        ...
        queue.push(function (messageId, message, queueDone) {
            if (messageId === Protocol.serverGoneMessageId) {
            ...
            if (!self[messageId]) {
                queueDone();
                fail(new Error("Unexpected message " + messageId));
                return;
            }
            try {
                self[messageId](message, queueDone);
            } catch (err) {
                fail(err);
            }
        });
